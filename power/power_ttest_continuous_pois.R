...
generate.data.norm <- function(number.of.seeds, sample.size, AA, BB, r, ef){
  ...
  order.table$num1_uni <- pnorm(order.table$num1)
  order.table$num1_pois <- qpois(order.table$num1_uni, lambda=2)
  order.table$num1_pois <- ifelse(order.table$char1=="A", order.table$num1_pois+ef, order.table$num1_pois)
...
sim <- function(n, iter, number.of.seeds, sample.size, AA, BB, r, ef, label){
  ...
  for(i in 1:n){
    ...
    #run test
    res <- randomize.norm(iter  , "char1", "num1_pois", net)
    #save result
...
set.seed(20+ A + B + r + ef)

#simulations to run
case1 <- sim(n,1000, 5, 500, A,B, r,ef,label)

saveRDS(case1, paste0("/projectnb/cbs/samalate/RDS/power/results/power_cont_pois",A,"_", B, "_", r,"_", n, "_", ef, ".rds"))
